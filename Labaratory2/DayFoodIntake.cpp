...
DayFoodIntake::DayFoodIntake(const unsigned int& countIntake)
	...
	for (int i = 0; i < countIntake; i++) {
		dynamincProteinArray[i] = NEGATIVE_VALUE;
		dynamincFatArray[i] = NEGATIVE_VALUE;
		dynamincCarbohydrateArray[i] = NEGATIVE_VALUE;
	}
}
...
{
	for (int i = 0; i < countIntake; i++) {
		if (dynamincProteinArray[i] != NEGATIVE_VALUE && dynamincFatArray[i] != NEGATIVE_VALUE && dynamincCarbohydrateArray[i] != NEGATIVE_VALUE)
		{
